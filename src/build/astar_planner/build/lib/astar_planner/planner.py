...
import numpy as np
from astar import AStar  # Importa tu clase AStar existente
...
class AStarPlanner(Node):
    ...
    def plan_path(self):
        if self.map_data is None or self.goal_pose is None:
            self.get_logger().warning("No se puede planificar: falta mapa o objetivo.")
            return
        ...
        # Ejecutar A*
        astar = AStar(map_array)
        path = astar.astar(start_idx, goal_idx, treshold=50)  # Ajusta el umbral según tu mapa
